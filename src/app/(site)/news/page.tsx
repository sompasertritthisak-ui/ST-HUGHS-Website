...
export default async function NewsPage({ searchParams }: { searchParams: Search }) {
  ...
  return (
    <>
      <PageHero eyebrow="News and insights" title={<>What is <span className="italic text-brand-soft">happening.</span></>} lede="Announcements, partnerships and academic updates from the college — each article links to the programme or university it concerns." />
        ...
        {categories.length > 1 ? (
          <nav aria-label="Filter by category" className="mb-12 border-y border-line py-6">
            <ul className="flex flex-wrap gap-2">
              <li>
                <Link href="/news#articles" className={cn(chip, !category ? "border-route text-brand-soft" : "border-line-strong text-fg-muted hover:border-fg hover:text-fg")} aria-current={!category ? "true" : undefined}>
                  All
                </Link>
              </li>
              {categories.map((c) => (
                <li key={c}>
                  <Link href={`/news?category=${c}#articles`} className={cn(chip, category === c ? "border-route text-brand-soft" : "border-line-strong text-fg-muted hover:border-fg hover:text-fg")} aria-current={category === c ? "true" : undefined}>
            ...
            {events.map((e) => (
              <li key={e.id} className="grid gap-2 py-5 sm:grid-cols-12 sm:gap-6">
                ...
                <div className="text-sm text-fg-muted sm:col-span-3">
                  ...
                  {e.registrationUrl ? (
                    <a href={e.registrationUrl} target="_blank" rel="noopener noreferrer" className="ml-3 text-brand-soft underline underline-offset-4">
                      Register
                    </a>
                  ) : null}
                </div>
